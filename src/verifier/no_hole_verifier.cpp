...
bool NoHoleVerifier::verify(ConfigData &config_data, GraphData &graph_data, std::vector<int> &solution, int solution_span)
{
    ...
    if (!verify_span(graph_data, solution, solution_span))
        return false;
    if (!verify_no_hole(config_data, graph_data, solution, solution_span))
        return false;

    return true;
}

bool NoHoleVerifier::verify_no_hole(ConfigData &config_data, GraphData &graph_data, std::vector<int> &solution, int solution_span)
{
